...
int main() {
    ...
    FILE* fptr = fopen("customers.bin","w");
    fwrite(&(db.customer_count), sizeof(int), 1, fptr);

    fwrite(db.customers, sizeof(Customer), db.customer_count, fptr);

    FILE* rec_fptr = fopen("reciepts.bin","w");
    fwrite(&(db.reciept_count), sizeof(int), 1, rec_fptr);

    fwrite(db.reciepts, sizeof(Reciept), db.reciept_count, rec_fptr);
