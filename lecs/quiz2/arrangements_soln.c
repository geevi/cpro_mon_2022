...
int count_arrangements(int n, int k) {
  ...
  return k==1? n : (n-k+1)*count_arrangements(n,k-1);
}
...
PermList enumerate_arrangements(
  ...
  if (k == 1) {
    ...
    // $\{1,..,n\}.$ (5 marks)
    int an = count_arrangements(n, k-1);
    int c = 0;
    PermList A = enumerate_arrangements(n, k-1);
    for (int i=0; i < an; i++) {
      int elements[n - k + 1];
      find_elements_not_in_row(A[i], n, k-1, elements);
      for (int j=0; j < n-k + 1 ; j++) {
        insert_and_copy(A[i],k-1, elements[j], B[c++]);         
      }
    }
  }
  return B;
