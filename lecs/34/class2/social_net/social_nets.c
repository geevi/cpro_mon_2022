...
bool check_degree_of_seperation(struct Person* p, struct Person* q, int degree) {
    if (degree == 1) {
        ...
        bool any_friend_within_degree = false;
        for (int i =0 ; i < p->count_friends; i++) {
             any_friend_within_degree = any_friend_within_degree || 
                        check_degree_of_seperation(p->friends[i],q, degree-1);
        }
        return any_friend_within_degree;
    }
