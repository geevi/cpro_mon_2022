...
int main(int argc, char const *argv[]) {
    ...
    db.customers = read_from_file(f_customers, db.customers, sizeof(Customer));

    // printf("read customers\n");

    FILE* f_reciepts = fopen(argv[2], "r");
    db.reciepts = read_from_file(f_reciepts, db.reciepts, sizeof(Reciept));
    ...
    while(true) {
        ...
        switch (option)
        {
            ...
            write_to_file(f_customers,db.customers, sizeof(Customer), db.customer_count);


            FILE* f_reciepts = fopen(argv[2], "w");

            write_to_file(f_reciepts,db.reciepts, sizeof(Reciept), db.reciept_count);
